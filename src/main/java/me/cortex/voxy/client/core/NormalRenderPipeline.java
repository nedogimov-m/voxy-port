...
public class NormalRenderPipeline extends AbstractRenderPipeline {
    ...
    protected void finish(Viewport<?> viewport, int sourceFrameBuffer, int srcWidth, int srcHeight) {
        ...
        // 1.20.1: getViewDistance().getValue() * 16 gives render distance in blocks
        float renderDistance = MinecraftClient.getInstance().options.getViewDistance().getValue() * 16f;
        boolean fogCoversAllRendering = viewport.fogParameters != null && viewport.fogParameters.environmentalEnd() < renderDistance;
        ...
        if (this.useEnvFog && viewport.fogParameters != null) {
            ...
            if (Math.abs(end-start)>1) {
                ...
                float clamped = Math.max(0, Math.min(1, endDistance*invEndFogDelta+startDelta));
                glUniform4f(4, invEndFogDelta, startDelta, clamped, 0);//
