...
import me.cortex.voxy.common.util.BitUtil;
...
public class RenderDataFactory {
    ...
    private void acquireNeighborData(WorldSection section, int msk) {
        ...
        if ((msk&16)!=0) {//-z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*4] = raw[BitUtil.expandInt(i,0b11111_00000_11111)|(0x1F<<5)];//pull the +z faces from the section
            }
            sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
        }
        if ((msk&32)!=0) {//+z
            ...
            var raw = sec._unsafeGetRawDataArray();
            for (int i = 0; i < 32*32; i++) {
                this.neighboringFaces[i+32*32*5] = raw[BitUtil.expandInt(i,0b11111_00000_11111)];//pull the -z faces from the section
            }
            sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
        }
    }
        ...
        for (int y = 0; y < 32; y++) {
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if (z == 30 && partialHasCount != 0) {//Hackfix for incremental count overflow issue
                    ...
                    //Clear the sum
                    sumA &= ~(BitUtil.expandLong(Integer.toUnsignedLong(partialHasCount), X_I_MSK)*0x1F);
                    sumB &= ~(BitUtil.expandLong(Integer.toUnsignedLong(partialHasCount)>>11, X_I_MSK)*0x1F);
                    sumC &= ~(BitUtil.expandLong(Integer.toUnsignedLong(partialHasCount)>>22, X_I_MSK)*0x1F);
                }
                ...
                {//Dont need this as can just increment everything then -1 in mask
                    ...
                    sumA += BitUtil.expandLong(imsk, X_I_MSK);
                    sumB += BitUtil.expandLong(imsk>>11, X_I_MSK);
                    sumC += BitUtil.expandLong(imsk>>22, X_I_MSK);
                }*/
